...
class Test01UserAPI:
    ...
    VALID_DATA_FOR_USER_CREATION = [
        (
            {"username": "TestUser_2", "role": "user", "email": "testuser2@yamdb.fake"},
            "",
        ),
        (
    ...
    def test_01_users_not_authenticated(self, client):
        ...
        assert response.status_code != HTTPStatus.NOT_FOUND, (
            "Эндпоинт `/api/v1/users/` не найден. Проверьте настройки в " "*urls.py*."
        )
    ...
    def test_04_02_users_get_search(self, user, admin_client, admin, django_user_model):
        url = "/api/v1/users/"
        ...
        users_count = django_user_model.objects.filter(username=admin.username).count()
    ...
    def test_05_03_users_post_response_has_data(self, admin_client):
        ...
        expected_keys = ("first_name", "last_name", "username", "bio", "role", "email")
    ...
    def test_05_04_users_post_user_superuser(
        ...
        users_after = django_user_model.objects.filter(email=valid_data["email"])
    ...
    def test_07_01_users_username_patch_admin(
        ...
        response = admin_client.patch(f"/api/v1/users/{user.username}/", data=data)
        assert response.status_code == HTTPStatus.OK, (
    ...
    def test_07_03_users_username_patch_user(
        ...
        user = django_user_model.objects.filter(username=user.username).first()
        self.check_user_data_not_changed_with_patch(user, "пользователя с ролью `user`")
    ...
    @pytest.mark.parametrize("data,messege", invalid_data_for_user_patch_and_creation)
    def test_10_02_users_me_has_field_validation(self, user_client, data, messege):
        url = "/api/v1/users/me/"
        request_method = "PATCH"
        response = user_client.patch(url, data=data)
        assert response.status_code == HTTPStatus.BAD_REQUEST, messege[0].format(
            url=url, request_method=request_method
        )
